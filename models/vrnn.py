...
class VRNN(nn.Module):
    ...
    #     feed_dict.update({
    #         self.initial_prev_z:
    #         np.ones(shape=(self.config.batch_size, self.config.n_state))
    #     })
    ...
    #         _, sum_op, base_loss = sess.run(
    #             [self.train_ops, self.summary_op, self.basic_loss], feed_dict)
    ...
    #             self.print_loss("%.2f" %
    #                             (train_feed.ptr / float(train_feed.num_batch)),
    #                             loss_names, [base_losses],
    #                             postfix='kl_w')

    #     # finish epoch!
    #     epoch_time = time.time() - start_time
    #     avg_losses = self.print_loss(
    #         "Epoch Done", loss_names, [base_losses],
